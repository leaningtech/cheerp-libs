...
[[cheerp::genericjs]] [[cheerp::jsexport]] client::Array* debugArray = new client::Array();
...
std::atomic<bool> mainThreadSleeps = false;

[[cheerp::genericjs]] void pushDebug(int nr)
{
	debugArray->push(nr);
}
...
long __syscall_futex(uint32_t* uaddr, int futex_op, ...)
{
	...
	switch (futex_op)
	{
		case FUTEX_WAKE:
		{
			...
			{
				mainThreadWaitAddress = 0;
				// Notify the main thread that it can wake up.
				mainThreadSleeps.store(false);
				val -= 1;
				threadsWokenUp += 1;
		...
		case FUTEX_WAIT:
		{
			...
			if (tid == 1) // TODO: improve to use actual browser main thread here.
			{
				...
				if (*uaddr != val)
					return EAGAIN;
				mainThreadSleeps.store(true);
				mainThreadWaitAddress.store(uaddr);
				while (mainThreadSleeps.load() == true)
				{
				}
...
void spawnThreadFromUtility()
{
	...
	worker->set_onmessage(waitForThreadCreation);//spawnThreadFromUtility);
...
void waitForThreadCreation()
{
	client::setTimeout(spawnThreadFromUtility, 200);
}
